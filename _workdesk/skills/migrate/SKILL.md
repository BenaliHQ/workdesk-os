...
Updates the WorkDesk OS control plane (`_workdesk/`) to the latest release. Operator-facing skill — most users won't be technical, so Claude leads with plain-language narration. The bash engine (`_workdesk/scripts/migrate.sh`) owns invariants; this skill orchestrates the conversation.

## Boundaries

- **Updates only `_workdesk/`** — your skills, rules, hooks, scripts, and the operator-profile schema. Never touches `personal/`, `atlas/`, `gtd/`, `intel/`, or `system/`.
- **Schema migrations** may rewrite specific files inside `_workdesk/` (e.g. `operator-profile.md` frontmatter when fields are renamed). Each migration is a versioned, reviewable script shipped with the release.
- **Backup is automatic** before any write, at `<vault>/.workdesk-backups/<timestamp>/`. Restore via `/migrate restore <id>` or by running `_workdesk/scripts/migrate.sh restore <id>`.
...
```
_workdesk/scripts/migrate.sh check
```
...
1. Read the operator's current file: `_workdesk/<path>`
2. Read the release version: `<staging>/workdesk/<path>`
3. Read the prior baseline (what the operator started from): `_workdesk/defaults/<path>`
...
```
_workdesk/scripts/migrate.sh apply <staging> <vault>/.workdesk-migrate-tmp/resolutions.json
```

Both paths come from the plan JSON (`staging`) and what you just wrote.

The engine:
1. Backs up `_workdesk/` to `<vault>/.workdesk-backups/<timestamp>/`
...
```
_workdesk/scripts/migrate.sh restore <backup-id>
```
...
If migrations partially fail mid-stream, the engine restores `_workdesk/` from backup but does NOT undo schema-migration writes that already landed. This is rare (migrations are idempotent and tested in CI), but if it happens, surface the migration name and which file it last touched so the operator can review manually.
