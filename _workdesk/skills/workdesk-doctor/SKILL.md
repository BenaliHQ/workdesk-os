...
Bootstrap is filesystem-only. Doctor proves runtime. Run after every bootstrap, after every `_workdesk/settings.json` edit, and any time a hook seems wrong.

## When to run

- First Claude Code session after bootstrap
- After editing `_workdesk/settings.json` or any hook script
...
Read `_workdesk/state/session-entry.md`. Verify it was written within the last 5 minutes (proof the hook fired this session). If absent or stale, the hook is broken or `_workdesk/settings.json` is misconfigured.
...
- If multiple files share a `session_id` and any has `complete: false`, `Stop` upsert mode is active — write `_workdesk/state/doctor.md` with `stop-fallback: enabled`
...
Confirm `_workdesk/state/session-entry.md` reflects current `system/transcripts/`, `system/intake/`, and `system/session-log/` reality:
...
Write `_workdesk/state/doctor.md`:
...
- Don't summarize without writing `_workdesk/state/doctor.md` — onboarding reads that file.
