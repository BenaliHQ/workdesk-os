...
description: Upgrade an action in gtd/actions/next/ into a full project folder under gtd/projects/. Re-runs the relevant POBO phases minimally and rewrites references.
---
...
All promotions land at `gtd/projects/{slug}/`. Projects live in GTD; there is no separate atlas-side initiative folder.
...
Create the 8-item folder under `gtd/projects/{slug}/`. Populate `_brief.md`, `_status.md`, `plan.md` from the mini-POBO output. The promoted action is the current next action; carry it into `_status.md`.

Frontmatter on `_brief.md`: `expected-cadence: biweekly`.

### 5. Rewrite the original action

The original `gtd/actions/next/{slug}.md` becomes a child of the new project:
...
Hooks fire `project-created` automatically. No manual logging.
