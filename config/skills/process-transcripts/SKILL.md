...
`atlas/meetings/{YYYY-MM-DD}-{topic-slug}.md` per `config/objects/meeting.md`:
...
- **Action items** — propose `gtd/actions/next/` creations via `[REVIEW]`, subject to flood guard. If the meeting clearly belongs to a project, set `parent:` to that project's brief (e.g. `[[gtd/projects/<slug>/_brief]]`) per `config/objects/action.md`; otherwise leave `parent:` empty.
